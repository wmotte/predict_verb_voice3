...
#
get_confusion_matrix <- function( model, newdata, test_label, num_classes = 3, minimal_probability = 0.8 )
{
    ...
    # Identify rows with >90% probability in any class
...
# add voice label
df_train$voice <- as.factor( substr( df_train$postag, 6, 6 ) )
df_test$voice <- as.factor( substr( df_test$postag, 6, 6 ) )
...
model <- xgb.train(
    ...
    max_depth = 24,            # Default: 6; reduce to simplify the model (12 ==> 24: better; 24 ==> 30: worse; 24 ==> 18: worse; 26: worse)
...
confusion_matrix <- get_confusion_matrix( model, newdata = dtest, test_label = test_label, num_classes = 3, minimal_probability = 0.33 )
...
# read data
df_mt <- readr::read_csv( 'out.02.prepare.train.test/df_matthew.csv.gz', show_col_types = FALSE )

# remove deponent verbs
df_mt <- df_mt[ - grep( 'deponent', df_mt$word ), ]

# make 'y' [active: 0, middle: 1, passive: 2, as required for input]
df_mt$y <- as.integer( as.factor( df_mt$group ) ) - 1

# labels
mt_label <- df_mt$y

# get data in matrix format
mt_data <- as.matrix( df_mt[ , grep( 'covariate_', colnames( df_mt ) ) ] )

# prepare
dmt <- xgb.DMatrix( data = mt_data, label = mt_label )

# 3. Predict on the Validation Set
pred_mt <- predict( model, newdata = dmt )

# 4. Convert Predictions to Class Labels
# Convert the predicted probabilities into a matrix (row: samples, col: classes)
pred_matrix <- matrix( pred_mt, ncol = num_classes, byrow = TRUE )

# Minimal probability to consider inspection
minimal_probability <- 0.8
...
# Filter predictions and actual labels for confident rows
filtered_predicted_labels <- max.col( pred_matrix[ confident_rows, , drop = FALSE ] ) - 1
filtered_actual_labels <- mt_label[ confident_rows ]
...
# Redirect output to a text file
sink( paste0( outdir, "/confusion_matrix_output_matthew.txt" ) )
...
# Plot
p_t_mt <- ggplot( conf_df, aes( x = Reference, y = Prediction, fill = Percent ) ) +
    geom_tile() +
    geom_text( aes( label = paste0( Freq, ' (', Label, ')' ) ), color = "white" ) +
    labs( x = "Actual", y = "Predicted", fill = "Percent" ) +
    theme_minimal() +
...
# ggplot
ggsave( plot = p_t_mt, file = paste0( outdir, '/confusion_plot_matthew_validation_set.png' ), dpi = 600, width = 4, height = 4, bg = 'white' )
